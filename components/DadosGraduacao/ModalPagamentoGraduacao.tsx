...
import { Smartphone, Building, Upload, Check, Copy, CreditCard, X } from "lucide-react";
import toast from "react-hot-toast";
import { Graduacao, getGraduacoes } from "../../lib/graduacao-actions";
...
interface ModalPagamentoGraduacaoProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: () => void;
}

const ModalPagamentoGraduacao: React.FC<ModalPagamentoGraduacaoProps> = ({ 
  isOpen, 
  onClose,
  onSuccess 
}) => {
  ...
  useEffect(() => {
    const fetchGraduacao = async () => {
      try {
        ...
        if (data?.length) setGraduacao(data[0]);
      } catch (error) {
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2">
                  <div className="bg-white dark:bg-gray-900 rounded-2xl p-6">
                    ...
                    {metodo === "transferencia" && <CompactPaymentMethods valor={graduacao?.preco || 0} />}
...
export default ModalPagamentoGraduacao;